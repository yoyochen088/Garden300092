...
async function drawShowcaseToCanvas(member, flowers, cardWidth) {
  ...
  //ctx.fillText('十里桃花', W - PAD, HEADER_H / 2 - 8);
  ctx.font = '10px Microsoft JhengHei, sans-serif';
  //ctx.fillText('300092', W - PAD, HEADER_H / 2 + 8);
  ...
  try {
    if (isIOS) {
      ...
      imgContainer.innerHTML = `
        ...
        <div style="margin-top:12px;color:#9e6b7e;font-size:13px;padding:10px;background:rgba(233,30,140,0.08);border-radius:8px;">
          📱 長按圖片 → 儲存至相片
        </div>`;
      btn.textContent = '🔄 重新生成';
      btn.onclick = () => location.href = 'flower-showcase.html';
